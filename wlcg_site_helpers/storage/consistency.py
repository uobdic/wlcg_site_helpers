...
def get_cache_files(cache_path):
    ...
    cache_files = ['local_entries.csv', 'grid_entries.csv',
                   'local_but_not_grid.csv', 'grid_but_not_local.csv']
...
def get_diff(df1, df2, cache='diff.csv'):
    ...
    df1_only = df1_only.rename(
        columns={'size_x': 'size', 'date_x': 'date', 'ftype_x': 'ftype'})
...
def diff_local_but_not_grid(entries_grid, entries_local, cache_path='/tmp'):
    ...
    local_but_not_grid = get_diff(
        entries_local, entries_grid, cache_diff_local_grid)
    msg = '{0} items on local FS but not on grid'.format(
        len(local_but_not_grid))
    logger.info(msg)
...
def diff_grid_but_not_local(entries_grid, entries_local, cache_path='/tmp'):
    ...
    grid_but_not_local = get_diff(
        entries_grid, entries_local, cache_diff_grid_local)
    msg = '{0} items on grid but not on local FS'.format(
        len(grid_but_not_local))
    logger.info(msg)
...
def check(grid_path, local_path, localfs=hdfs, cache_path='/tmp'):
    ...
    entries_grid = grid_entries(grid_path, davix.ls, cache_path)

    local_but_not_grid = diff_local_but_not_grid(
        entries_grid, entries_local, cache_path)

    grid_but_not_local = diff_grid_but_not_local(
        entries_grid, entries_local, cache_path)
